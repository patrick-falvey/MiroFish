...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // This ensures all edges are distributed in the same reference frame without directional overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravity toward center to cluster independent nodes
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset from line direction, proportional to distance, ensuring visible curves
    // More edges means larger offset proportion
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, increase 5% per edge
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('end', (event, d) => {
        // Only let simulation wind down if actual drag occurred
        if (d._isDragging) {
