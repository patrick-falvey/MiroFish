            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content ready, but the section may not be fully complete yet) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When it's the final answer, show a special notice -->
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: - Selected Parent_601 (index 0): As a parent group representative...
        if (!headerMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compatibility: only one platform marker present)
      ...
      // Extract key quotes (compatible with various quotation mark formats)
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No current active memories available'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: first question returns the full answer, rest return empty
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 headings (## xxx), since section titles are displayed in the outer wrapper
  ...
  // Clean up <br> before paragraph start tags that immediately follow block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: keep numbering incremental when single-item <ol> elements are separated by paragraph content
...
const extractFinalContent = (response) => {
  ...
  // If content is long and contains markdown formatting, try removing the thought process and return
