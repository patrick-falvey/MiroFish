    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Launching...' : 'Generate Results Report' }} 
...
const runStatus = ref({})
const allActions = ref([]) // All actions (incremental accumulation, capped at 500)
...
// Computed
// Display actions in chronological order (newest at the bottom) - only render visible slice
...
const doStartSimulation = async () => {
  ...
  // Reset all state first to avoid being affected by the previous simulation
  resetAllState()
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect whether simulation is completed (via runner_status or platform completion status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check: if the backend hasn't updated runner_status yet, but platforms report completion
