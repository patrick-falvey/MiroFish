...
import { Step5Interaction } from '../components/wizard/step-5-interaction';
...
export function InteractionView() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      addLog('Loading interaction data...');
      try {
    ...
    <div className="flex h-screen flex-col">
      <header className="flex h-[60px] items-center justify-between border-b border-brand-100 bg-white px-4">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-xs text-brand-500">Step 5/5 · Interaction</span>
          <span className={cx('h-2 w-2 rounded-full', currentStatus === 'ready' ? 'bg-green-500' : 'bg-brand-500')} />
        </div>
      </header>

      <div className="flex flex-1 overflow-hidden">
        <div className="h-full overflow-hidden transition-all duration-500" style={leftPanelStyle}>
          <GraphPanel graphData={graphData} loading={graphLoading} currentPhase={5}
