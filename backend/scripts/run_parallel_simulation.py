...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout/stderr streams to UTF-8 (fix console character encoding issues)
...
# Load the .env file from the project root directory (contains LLM_API_KEY and other settings)
...
class ParallelIPCHandler:
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get the environment and agent_graph for a specific platform.
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        """
        # If a specific platform was requested, interview on that platform only
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    # Common placeholder values in .env examples (should not be treated as real config)
    placeholder_values = {"", "your_api_key_here", "your_base_url_here", "your_model_name_here"}

    # Clean up placeholder values to avoid accidentally triggering the boost LLM branch
    ...
    if use_boost and has_boost_config:
        ...
        # Use general configuration — support Codex OAuth auth mode
    ...
    # Fall back to config if model name not set in .env
    if not llm_model:
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("Missing API Key configuration. Please set LLM_API_KEY in the project root .env file, or use LLM_AUTH_MODE=codex")
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to ensure proper shutdown on SIGTERM/SIGINT.
    ...
    2. Give the program a chance to cleanly release resources (close database, environment, etc.)
    3. Then exit
    """
