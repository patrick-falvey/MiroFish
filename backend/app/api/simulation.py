...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - completed: Run completed, meaning preparation was completed earlier
    ...
    try:
        ...
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # Skip edge info for faster results
            )
            ...
            # Failure doesn't block the flow; background task will re-fetch
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if the simulation is running or completed, it will stop and clean run logs first
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activity during simulation (posting, commenting, liking, etc.) is updated to the Zep graph in real-time
        ...
        - Uses batch update mechanism to reduce API call volume
    ...
    try:
        ...
        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"Invalid platform type: {platform}. Available options: twitter/reddit/parallel"
            }), 400
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is currently running. Please call the /stop endpoint first, or use force=true to force a restart."
                            }), 400
                ...
                # Process doesn't exist or has ended, reset state to ready
                logger.info(f"Simulation {simulation_id} preparation complete, resetting state to ready (previous state: {state.status.value})")
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",                    // Optional, specify platform (twitter/reddit)
                                                       // If not specified: dual-platform simulation interviews on both platforms
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                             // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What is your opinion on B?"  // No platform uses default
                }
            ],
    ...
    try:
        ...
        # Optimize each interview item's prompt with prefix to prevent Agent from calling tools
...
def close_simulation_env():
    ...
    while this endpoint allows the simulation to gracefully close and exit.
