...
Simulation report generation using LangChain + Zep with the ReACT pattern
...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Record section content generation complete (content only, does not indicate the entire section is finished)"""
        self.log(
    ...
    def log_section_full_complete(
        ...
        """
        Record that section generation is fully complete

        The frontend should listen for this log to determine if a section is truly finished and to get the full content
        """
        self.log(
    ...
    def log_report_complete(self, total_sections: int, total_time_seconds: float):
        """Record that report generation is complete"""
        self.log(
...
[Return Content]
- Relevant factual texts (can be quoted directly)
- Core entity insights
- Relationship chain analysis"""

TOOL_DESC_PANORAMA_SEARCH = """\
[Broad Search - Get a Panoramic View]
This tool is used to get a complete overview of simulation results, particularly suited for understanding event evolution. It will:
...
A lightweight quick retrieval tool, suited for simple and direct information queries.
...
This is not LLM simulation, but calls the actual interview interface to get original answers from simulation Agents.
...
[Use Cases]
- Need to understand event perspectives from different roles (How do students see it? How does the media see it? What do officials say?)
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports," with a "God's-eye view" of the simulated world — you can observe the behavior, statements, and interactions of every Agent in the simulation.
...
- Agent behavior and statements in the simulated world are predictions of future population behavior
...
3. [Language Consistency - Quoted Content Must Be Translated to the Report Language]
   - Content returned by tools may contain English or mixed language expressions
   - If the simulation requirements and source materials are in Chinese, the report must be written entirely in Chinese
   - When quoting English or mixed-language content from tools, you must translate it into fluent Chinese before including it in the report
...
Weibo, as the primary scene for public sentiment, served as the core channel for breaking news:
...
class ReportAgent:
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only attempted when Format 1 does not match, to avoid false matches on JSON in body text
    ...
    def _generate_section_react(
        ...
        conflict_retries = 0  # Consecutive conflict count for simultaneous tool calls and Final Answer
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returned None; break out of loop and enter forced finalization
                break
            ...
            # ── Conflict handling: LLM output both a tool call and Final Answer simultaneously ──
    ...
    def generate_report(
        ...
        # If no report_id was provided, auto-generate one
        if not report_id:
