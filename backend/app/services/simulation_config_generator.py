...
Uses a step-by-step generation strategy to avoid failures from overly long output:
...
# US Mountain Time activity config (Boise, MT — UTC-7)
US_MOUNTAIN_TIMEZONE_CONFIG = {
    ...
    # Evening peak (most active — after work/dinner)
    ...
    "activity_multipliers": {
        ...
        "morning": 0.4,    # Gradual morning ramp-up
        "work": 0.7,       # Moderate during work hours
        "peak": 1.5,       # Evening peak after work
        "night": 0.5       # Winding down late evening
    }
}
...
class TimeSimulationConfig:
    """Time simulation config (US Mountain Time — Boise, UTC-7)"""
    # Total simulated hours
    total_simulation_hours: int = 72  # Default 72 hours (3 days)

    # Simulated minutes per round — default 60 min (1 hour)
    minutes_per_round: int = 60

    # Agent activation range per simulated hour
    agents_per_hour_min: int = 5
    agents_per_hour_max: int = 20

    # Evening peak (6-9 PM MT — most active after work)
    peak_hours: List[int] = field(default_factory=lambda: [18, 19, 20, 21])
    peak_activity_multiplier: float = 1.5

    # Off-peak / late night (12-5 AM MT — minimal activity)
    off_peak_hours: List[int] = field(default_factory=lambda: [0, 1, 2, 3, 4, 5])
    off_peak_activity_multiplier: float = 0.05

    # Morning ramp-up (6-8 AM MT)
    morning_hours: List[int] = field(default_factory=lambda: [6, 7, 8])
    morning_activity_multiplier: float = 0.4

    # Work hours (9 AM - 5 PM MT)
...
class PlatformConfig:
    ...
    # Viral threshold (number of interactions before triggering viral spread)
    viral_threshold: int = 10
    
    # Echo chamber strength (degree to which similar viewpoints cluster together)
...
class SimulationConfigGenerator:
    ...
    Uses an LLM to analyze simulation requirements, document content, and
    knowledge-graph entity information, then automatically generates optimal
    ...
    AGENT_SUMMARY_LENGTH = 300           # Entity summary within Agent config
    ...
    def _call_llm_with_retry(self, prompt: str, system_prompt: str) -> Dict[str, Any]:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                temp = 0.7 - (attempt * 0.1)  # Lower temperature on each retry
...
### Core principles (adjust based on event type and target audience):
- Default timezone is US Mountain Time (Boise, UTC-7)
- 12-5 AM: minimal activity (multiplier 0.05)
- 6-8 AM: gradual morning ramp-up (multiplier 0.4)
- 9 AM - 5 PM: moderate work-hours activity (multiplier 0.7)
- 6-9 PM: evening peak — most active after work/dinner (multiplier 1.5)
- 10-11 PM: activity winds down (multiplier 0.5)
- General pattern: late-night low → morning ramp → work-hours moderate → evening peak
- **Important**: These defaults are starting points — adjust for the specific audience
  - e.g., college students may peak 9-11 PM; media personalities active all day; government offices only during work hours
  - e.g., breaking news may drive late-night discussion, so off_peak_hours can be shortened
...
Field descriptions:
- total_simulation_hours (int): Total simulated duration, 24-168 hours. Shorter for breaking news, longer for sustained topics.
...
- peak_hours (int array): Peak activity hours — adjust for the target audience
- off_peak_hours (int array): Low activity hours — typically late night / early morning
- morning_hours (int array): Morning ramp-up hours
- work_hours (int array): Work hours
- reasoning (string): Brief explanation of why this configuration was chosen"""

        system_prompt = "You are a social media simulation expert. Return pure JSON. Time configuration should default to US Mountain Time (Boise, UTC-7) activity patterns unless the scenario specifies a different region."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Get default time config (US Mountain Time — Boise, UTC-7)"""
        return {
            "total_simulation_hours": 72,
            "minutes_per_round": 60,
            ...
            "morning_hours": [6, 7, 8],
            "work_hours": [9, 10, 11, 12, 13, 14, 15, 16, 17],
            "reasoning": "Default US Mountain Time activity pattern (1-hour rounds)"
        }
...
## Task
Generate activity configurations for each entity. Guidelines:
- **Schedule follows US Mountain Time patterns**: minimal activity 0-5 AM, most active 6-9 PM
...
- **Public figures/Experts**: moderate activity (0.4-0.6), moderately high influence (1.5-2.0)
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<list of active hours, following US Mountain Time patterns>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configurations should follow US Mountain Time activity patterns."
        ...
        for i, entity in enumerate(entities):
            ...
            # If LLM did not generate config, use rule-based generation
            if not cfg:
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single Agent config using rules (US Mountain Time patterns)"""
